...
static int
lcd160x_open(struct inode *dev_node, struct file *instance)
{
	SD *data = NULL;
...
static int
lcd160x_close(struct inode *dev_node, struct file *instance)
{
	SD *data = NULL;
...
static long
lcd160x_ioctl(struct file *instance, unsigned int cmd,
		  unsigned long __user arg)
{
	...
	case IOCTL_SET_I2C_CLIENT:
		...
		struct i2c_board_info info = {
			I2C_BOARD_INFO("pcf8574_lcd160x", user_data.addr),
		};
...
{
	pr_info("i2c_lcd160x_init called\n");
...
module_init(i2c_lcd160x_init);
module_exit(i2c_lcd160x_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("pcf8574 base lcd1602 driver");
